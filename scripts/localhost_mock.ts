...
const main = async () => {
  ...
  console.log(
    ...
    chalk.red("2 ETH"),
    "from address",
    chalk.red(signers[1].address),
    "in pool as challenger"
  );
  console.log(
    "Staked ",
    chalk.red("1 ETH"),
    "from address",
    chalk.red(signers[0].address),
    "in pool as staker"
  );
  console.log(
    "Staked ",
    chalk.red("3 ETH"),
    "from address",
    chalk.red(signers[2].address),
    "in pool as staker"
  );
  console.log(
    chalk.cyan(
      "This test can be made to fail if checkTest is called on an even block (just call it twice in a row)"
    )
  );
  ...
  console.log(
    "This test is",
    chalk.magenta("failing"),
    ". Fail triggered by",
    chalk.cyan(signers[3].address)
  );
