...
contract AnteIronPegTest is AnteTest("IRON Peg Holds Test") { 
    ...
    uint public constant MAX_PRICE = 11 * 10e11;
    ...
    uint public lastCumPrice;
    uint public lastTestedTwap;
    ...
    constructor () {
        ...
        testedContracts = [quickswapPool, 
            ...
            0xD078B62f8D9f5F69a6e6343e3e1eC9059770B830];//Zap pool        
    }


    function checkpoint() public {
        require(UniswapV2OracleLibrary.currentBlockTimestamp() - lastCheckpointTime > MAX_PERIOD, 
    ...
    function twap() public view returns (uint) {
        // only calculate if checkpoint exists
        if (UniswapV2OracleLibrary.currentBlockTimestamp() - lastCheckpointTime > MAX_PERIOD) {
            return 0;
        }

        (uint currentCumPrice,, uint32 currentTimestamp) = 
            UniswapV2OracleLibrary.currentCumulativePrices(quickswapPool);

        // overflow is desired
        uint32 timeElapsed = currentTimestamp - lastCheckpointTime;
        FixedPoint.uq112x112 memory _twap = 
            FixedPoint.uq112x112(uint224((currentCumPrice - lastCumPrice) / timeElapsed));
